  ...
  try {
    ...
    if (walletProvider.wallet.isConnected()) {
      ...
      for (const token in tokenTypes) {
        ...
        qsaInnerText(`#${token} .wallet-balance span.near.balance`, removeDecZeroes(walletAvailable));

        if (Number(walletAvailable.replace(",", "")) > 1) {
          qs(`#${token} .wallet-balance a .max`).style.display = "inline";
        }
          ...
          case "unstake": {
            ...
            if (receiver) {
              switch (receiver) {
                ...
                case CHEDDAR_POOL_CONTRACT_NAME: {
                  showSuccess(`Unstaked ${convertToDecimals(data, tokens.cheddar.metaData!.decimals, 2)} ${tokens.cheddar.metaData!.symbol}`)
                  break;
                }
                case NEXT_POOL_CONTRACT_NAME: {
                  showSuccess(`Unstaked ${convertToDecimals(data, tokens.next.metaData!.decimals, 2)} ${tokens.next.metaData!.symbol}`)
                  break;
                }
              }
            }
